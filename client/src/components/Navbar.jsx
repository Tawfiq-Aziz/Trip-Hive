...
const Navbar = () => {
    ...
    return (
            <nav className={`fixed top-0 left-0 w-full flex shadow-md items-center
            ...
            duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700  backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                ...
                { user && (
                    ...
                    cursor-pointer text-black transition-all`} onClick={()=> isOwner ? navigate('/owner') : setShowHotelReg(true)}>
                ...
                <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-gray-800 transition-all duration-500 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                    ...
                    {user && <button className="border px-4 py-1 text-sm font-black 
                    rounded-full cursor-pointer text-black transition-all" onClick={()=> 
                    navigate('/owner')}>
                        Dashboard
                    </button>}

                    {!user && <button onClick={openSignIn} className="bg-black text-white px-8 
                    py-2.5 rounded-full transition-all duration-500">
                        Login
                    </button>}
                </div>
            </nav>
